...
const DaysTimePickerModal = ({ isVisible, onConfirm, onCancel }) => {
  ...
  const handleConfirm = () => {
    ...
    if (!isNaN(date) && dayOfWeek) {
      ...
      const selectedHour = parseInt(startTime.hour) + (startTime.period === "PM" ? 12 : 0);
      const selectedMinute = parseInt(startTime.minute);
      ...
      const selectedTime = `${startTime.hour}:${startTime.minute} ${startTime.period}`;
...
const styles = StyleSheet.create({
  ...
  closeButton: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: 'grey', // You can choose a color that fits your design
    borderRadius: 5,
  },
});
