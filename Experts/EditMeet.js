...
const handleFileChange = (event) => {
  ...
  const handleSubmit = async () => {
     ...
     try {
       ...
       const formattedTime = moment(selectedDateTime).format('YYYY-MM-DD HH:mm:ss');
        ...
        // Check if all state values are populated
        console.log("Data before sending:", { topic, description, formattedTime, duration, materialFiles });
       ...
       // Material files, if they are optional or can be ignored for testing
       materialFiles.forEach((file, index) => {
         jsonData[`material_hub_${index}`] = file;
       });

       const response = await axios.put(`${apiUrl}/api/expert/newhubmeeting/edit/${ID}`, jsonData, {
         headers: {
           Authorization: `Bearer ${token}`,
        ...
        if (response.status === 200) {
           setAlertMessage(t('Meeting updated successfully'));
           setIsSuccess(true);
        } else {
           setAlertMessage(t('Failed to update Meeting'));
           setIsSuccess(false);
        }
     } catch (error) {
  ...
  const hideAlert = () => {
    ...
    // Only perform onClose actions and refresh the page if the meeting update was successful
    if (isSuccess) {
      setAlertVisible(false);
      setIsVisible(false);
      onClose();
...
>
    <Text style={styles.buttonTextplus}>
        {submitloading ? t("Loading...") : t("Save Changes")}
    </Text>
</TouchableOpacity>
